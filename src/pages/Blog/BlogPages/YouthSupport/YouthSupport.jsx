...
// This component renders a full blog post on the topic of youth support through school-based social work
const YouthSupport = () => {
  // Access the location object to retrieve state passed via route navigation (e.g., title and image)
  const location = useLocation();
  const { title, imgSrc } = location.state || {}; // Destructure route state safely with a fallback

  // Blog content is passed as children to the BlogPostPage component
  const children = (
    <article>
      <header>
        <p>
  ...
  return (
    <>
      {/* Renders the blog post layout with dynamic content */}
      <BlogPostPage
        heading={title} // Heading passed from route state
        imgSrc={imgSrc} // Image passed from route state
